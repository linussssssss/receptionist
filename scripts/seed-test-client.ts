...
async function main() {
    ...
    const twilioNumber = process.env.TWILIO_PHONE_NUMBER || '+49xxxxxxxxxxxx'; //Has to be replaced with number obviously

    const client = await prisma.client.upsert({
        where: { phoneNumber: twilioNumber },
        update: {},
        create: {
            name: 'Test Zahnarzt Praxis',
            industry: 'dental',
            phoneNumber: twilioNumber,
            email: 'test@example.com', //Has to be replaced too
            businessHours: {
            ...
            greetingMessage: 'Guten Tag, Sie haben die Zahnarztpraxis Dr. Test erreicht. Wie kann ich Ihnen helfen?',
